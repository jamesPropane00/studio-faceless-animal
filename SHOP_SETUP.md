...
Apply migrations `039_shop_digital_products_and_admin_lockdown.sql`, `040_shop_platform_sessions.sql`, `041_shop_reservation_and_payment_hardening.sql`, `042_shop_seo_and_marketplace_foundation.sql`, and `043_shop_spring_products.sql` after migration 038. Shop administration uses the existing Faceless Animal website login, not a second Supabase Auth account. After a valid normal login, `smooth-endpoint` issues an opaque seven-day admin token only for `jdot00` or `jamespropane00`. `dynamic-function` verifies that token on every request. Migration 040 also removes browser read access to password, recovery and email columns. Migration 041 releases expired holds when the store loads and automatically refunds a verified Stripe payment if its ten-minute inventory reservation already expired. Migration 042 adds permanent product slugs, search metadata, GTIN/MPN/category fields, search indexes and connector-neutral eBay/Facebook listing records. Migration 043 adds Spring fulfillment links and creates the first supplied Spring sticker listing.
...
Spring products receive searchable Faceless Supply product pages, but their purchase buttons open the official Spring listing. They are never added to the Stripe cart. Spring remains responsible for variants, live inventory, payment, production, shipping, returns and customer support. Spring sales remain in the Spring account rather than the Faceless Supply Stripe order dashboard.
