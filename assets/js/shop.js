...
async function load(){
 ...
 const {data,error}=await supabase.from("products").select("id,title,description,price_cents,quantity,sku,condition,category,shipping_price_cents,local_pickup,state,product_images(public_url,alt_text,sort_order)").eq("published",true).order("created_at",{ascending:false});
...
function renderProducts(){
 ...
 $("#product-grid").innerHTML=list.map(p=>{const sold=p.state==="sold"||p.quantity<1,low=!sold&&p.quantity<=3;return `<article class="product-card">
 <div class="product-image" data-detail="${p.id}">${image(p)?`<img src="${safe(image(p))}" alt="${safe(p.title)}" loading="lazy">`:`<div class="no-image">FA</div>`}${sold?`<span class="badge sold">Sold out</span>`:low?`<span class="badge low">Only ${p.quantity} left</span>`:""}</div>
 <div class="product-copy"><span class="product-meta">${safe(p.category)} · ${safe(p.condition)}</span><h3>${safe(p.title)}</h3><span class="price">${money(p.price_cents)}</span>
...
function add(id,buy=false){const p=productById(id);if(!p||p.quantity<1)return;const line=cart.find(i=>i.id===id);if(line)line.quantity=Math.min(line.quantity+1,p.quantity);else cart.push({id,quantity:1});save();openCart();if(buy)$("#checkout-form input[name=name]").focus()}
function renderCart(){
 ...
 const fulfill=$('input[name="fulfillment"]:checked')?.value||"shipping";
 const total=cart.reduce((n,i)=>{const p=productById(i.id);return n+(p.price_cents+(fulfill==="shipping"?p.shipping_price_cents:0))*i.quantity},0);
...
function detail(id){const p=productById(id);$("#product-detail").innerHTML=`<div class="detail-grid"><div class="detail-gallery">${image(p)?`<img src="${safe(image(p))}" alt="${safe(p.title)}">`:""}</div><div class="detail-copy"><p class="eyebrow">${safe(p.category)} · ${safe(p.condition)}</p><h2>${safe(p.title)}</h2><p class="price">${money(p.price_cents)}</p><p class="detail-description">${safe(p.description)}</p><p class="product-meta">${p.local_pickup?"Shipping or Providence pickup":"Ships to you"} · SKU ${safe(p.sku)}</p><div class="card-actions"><button data-add="${p.id}">Add to bag</button><button class="primary" data-buy="${p.id}">Buy now</button></div></div></div>`;$("#product-modal").hidden=false}
