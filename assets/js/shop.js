...
function renderProducts(){
 ...
 <div class="product-copy"><span class="product-meta">${safe(p.category)} · ${fanvue?"Fanvue exclusive":spring?"Spring fulfillment":p.product_kind==="physical"?safe(p.condition):"Instant download"}</span><h3><a href="${productUrl(p)}">${safe(p.title)}</a></h3><span class="price">${spring?"From ":""}${fanvue&&p.price_cents<1?"Exclusive access":money(p.price_cents)}</span>
...
function detail(id){const p=productById(id),external=externalUrl(p),spring=p.fulfillment_provider==="spring"&&external,fanvue=p.fulfillment_provider==="fanvue"&&external,digital=!external&&p.product_kind!=="physical";$("#product-detail").innerHTML=`<div class="detail-grid"><div class="detail-gallery">${image(p)?`<img src="${safe(image(p))}" alt="${safe(p.title)}">`:""}</div><div class="detail-copy"><p class="eyebrow">${safe(p.category)} · ${fanvue?"18+ on Fanvue":spring?"Made to order by Spring":digital?"Digital download":safe(p.condition)}</p><h2>${safe(p.title)}</h2><p class="price">${spring?"From ":""}${fanvue&&p.price_cents<1?"Exclusive access":money(p.price_cents)}</p><p class="detail-description">${safe(p.description)}</p>${digital&&p.preview_url?`<p><a class="price" href="${safe(p.preview_url)}" target="_blank" rel="noopener">Preview →</a></p>`:""}<p class="product-meta">${fanvue?"Fanvue handles sign-in, age controls, payment, and access":spring?"Spring handles payment, production, shipping, and support":digital?"Download released after verified payment":p.local_pickup?"Shipping or Providence pickup":"Ships to you"} · SKU ${safe(p.sku)}</p><p><a class="product-page-link" href="${productUrl(p)}">Open permanent product page →</a></p>${external?`<div class="card-actions spring-action"><a class="primary" href="${safe(external)}" target="_blank" rel="noopener">${fanvue?"View and unlock on Fanvue (18+) →":"Choose options and buy on Spring →"}</a></div>`:`<div class="card-actions"><button data-add="${p.id}">Add to bag</button><button class="primary" data-buy="${p.id}">Buy now</button></div>`}</div></div>`;$("#product-modal").hidden=false}
