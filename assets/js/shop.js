...
async function load(){
 ...
 cart=cart.filter(i=>productById(i.id)?.fulfillment_provider!=="spring");save();renderProducts();
...
function renderProducts(){
 ...
 $("#product-grid").innerHTML=list.map(p=>{const spring=p.fulfillment_provider==="spring"&&springUrl(p),reserved=!spring&&p.state==="reserved",sold=!spring&&(p.state==="sold"||p.quantity<1),low=!spring&&!sold&&!reserved&&p.quantity<=3;return `<article class="product-card">
 <a class="product-image" href="${productUrl(p)}">${image(p)?`<img src="${safe(image(p))}" alt="${safe(p.title)}" loading="lazy">`:`<div class="no-image">FA</div>`}${spring?`<span class="badge">Made to order by Spring</span>`:reserved?`<span class="badge sold">Temporarily reserved</span>`:sold?`<span class="badge sold">Sold out</span>`:low?`<span class="badge low">Only ${p.quantity} left</span>`:""}</a>
 <div class="product-copy"><span class="product-meta">${safe(p.category)} · ${spring?"Spring fulfillment":p.product_kind==="physical"?safe(p.condition):"Instant download"}</span><h3><a href="${productUrl(p)}">${safe(p.title)}</a></h3><span class="price">${spring?"From ":""}${money(p.price_cents)}</span>
 ${spring?`<div class="card-actions spring-action"><a class="primary" href="${safe(spring)}" target="_blank" rel="noopener">Choose options on Spring →</a></div>`:`<div class="card-actions"><button data-add="${p.id}" ${sold?"disabled":""}>Add to bag</button><button class="primary" data-buy="${p.id}" ${sold?"disabled":""}>Buy now</button></div>`}</div></article>`}).join("")||"<p class='shop-status'>No pieces match that search.</p>";
}
function add(id,buy=false){const p=productById(id);if(!p||p.fulfillment_provider==="spring"||p.state==="sold"||p.state==="inactive"||p.quantity<1)return;const line=cart.find(i=>i.id===id);if(line)line.quantity=Math.min(line.quantity+1,p.quantity);else cart.push({id,quantity:1});save();openCart();if(buy)$("#checkout-form input[name=name]").focus()}
function renderCart(){
...
function detail(id){const p=productById(id),spring=p.fulfillment_provider==="spring"&&springUrl(p),digital=!spring&&p.product_kind!=="physical";$("#product-detail").innerHTML=`<div class="detail-grid"><div class="detail-gallery">${image(p)?`<img src="${safe(image(p))}" alt="${safe(p.title)}">`:""}</div><div class="detail-copy"><p class="eyebrow">${safe(p.category)} · ${spring?"Made to order by Spring":digital?"Digital download":safe(p.condition)}</p><h2>${safe(p.title)}</h2><p class="price">${spring?"From ":""}${money(p.price_cents)}</p><p class="detail-description">${safe(p.description)}</p>${digital&&p.preview_url?`<p><a class="price" href="${safe(p.preview_url)}" target="_blank" rel="noopener">Preview →</a></p>`:""}<p class="product-meta">${spring?"Spring handles payment, production, shipping, and support":digital?"Download released after verified payment":p.local_pickup?"Shipping or Providence pickup":"Ships to you"} · SKU ${safe(p.sku)}</p><p><a class="product-page-link" href="${productUrl(p)}">Open permanent product page →</a></p>${spring?`<div class="card-actions spring-action"><a class="primary" href="${safe(spring)}" target="_blank" rel="noopener">Choose options and buy on Spring →</a></div>`:`<div class="card-actions"><button data-add="${p.id}">Add to bag</button><button class="primary" data-buy="${p.id}">Buy now</button></div>`}</div></div>`;$("#product-modal").hidden=false}
