...
function renderProducts(){
 ...
 <a class="product-image" href="${productUrl(p)}">${image(p)?`<img src="${safe(image(p))}" alt="${safe(p.title)}" loading="lazy">`:`<div class="no-image">FA</div>`}${reserved?`<span class="badge sold">Temporarily reserved</span>`:sold?`<span class="badge sold">Sold out</span>`:low?`<span class="badge low">Only ${p.quantity} left</span>`:""}</a>
 <div class="product-copy"><span class="product-meta">${safe(p.category)} · ${p.product_kind==="physical"?safe(p.condition):"Instant download"}</span><h3><a href="${productUrl(p)}">${safe(p.title)}</a></h3><span class="price">${money(p.price_cents)}</span>
