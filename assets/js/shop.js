...
async function load(){
 ...
 const actionSlug=params.get("buy")||params.get("add");
 const actionProduct=products.find(product=>product.slug===actionSlug);
 if(actionProduct){add(actionProduct.id,params.has("buy"));history.replaceState({},"",location.pathname)}
}
...
function renderCart(){
 ...
 $("#shipping-fields").querySelectorAll("input").forEach(input=>input.required=fulfill==="shipping"&&input.name!=="line2");
...
document.querySelectorAll('input[name="fulfillment"]').forEach(r=>r.onchange=()=>{const ship=r.value==="shipping"&&r.checked;$("#shipping-fields").hidden=!ship;$("#shipping-fields").querySelectorAll("input").forEach(i=>i.required=ship&&i.name!=="line2");renderCart()});
