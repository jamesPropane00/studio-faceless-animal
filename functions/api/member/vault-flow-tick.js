...
export const onRequestPost = async (context) => {
  ...
  const elapsed = Number((body && body.elapsed) || 0);
  const readBalance = body && body.read_balance === true;

  if (!username || !ph) {
    return jsonResponse({ ok: false, error: 'Missing username or ph' }, 400);
  }

  let memberRow = null;

  const lookupRes = await fetch(
    `${SUPA_URL}/rest/v1/member_accounts?username=eq.${encodeURIComponent(username)}&select=username,display_name,plan_type,member_status,password_hash,credits_balance`,
    {
      method: 'GET',
      headers: {
        apikey: SERVICE_KEY,
  ...
  if (!memberRow) {
    const createRes = await fetch(`${SUPA_URL}/rest/v1/member_accounts`, {
      method: 'POST',
      headers: {
        apikey: SERVICE_KEY,
        Authorization: `Bearer ${SERVICE_KEY}`,
  ...
  if (!memberRow) {
    return jsonResponse({ ok: false, error: 'Could not find or create member' }, 500);
  }

  if (String(memberRow.password_hash || '') !== ph) {
  ...
  if (readBalance) {
    return jsonResponse(
      {
        ok: true,
        credits_balance: Number(memberRow.credits_balance || 0),
        generated: 0
      },
      200
    );
  ...
  if (!Number.isFinite(elapsed) || elapsed <= 0) {
    return jsonResponse({ ok: false, error: 'No elapsed time' }, 400);
  }

  const reward = Math.max(1, Math.floor(elapsed / 10));
  const currentBalance = Number(memberRow.credits_balance || 0);
  const nextBalance = Math.max(0, currentBalance + reward);

  const patchRes = await fetch(
    `${SUPA_URL}/rest/v1/member_accounts?username=eq.${encodeURIComponent(username)}`,
    {
      method: 'PATCH',
      headers: {
        apikey: SERVICE_KEY,
  ...
  if (!patchRes.ok) {
    const raw = await safeText(patchRes);
    return jsonResponse({ ok: false, error: `DB error${raw ? `: ${raw}` : ''}` }, 500);
  }

  const patchedRows = await patchRes.json();
  const row = Array.isArray(patchedRows) && patchedRows.length > 0
    ? patchedRows[0]
    : { credits_balance: nextBalance };
  ...
  return jsonResponse(
    {
      ok: true,
      credits_balance: Number(row.credits_balance || nextBalance),
      generated: reward
    },
    200
  );
